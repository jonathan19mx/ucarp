...
case "$1" in 
   ...
   echo -e "$(hostname) is now an active slave for PostgreSQL.\nBackups are stored in '$BACKUP_DIR/ha_backups'." | mail -s 
   "$(hostname) is now the PostgreSQL Slave" $CONTACT 
else                               
        ...
        UCARPIF=$(ifconfig | grep "$ucarp_addr " | grep -v grep) 
                ...
                -n 3 | tail -n 1)                        
                REMOTESLAVESTAT=$(sudo -u $PGUSER psql -h$REMOTE postgres -c "SELECT pg_last_xlog_receive_location()" 
                $PGUSER | head -n 3 | tail -n 1) 
                        ...
                        echo -e "\tWAL Sender Process found!" 
                          ...
                          head -n 3 | tail -n 1)   
                          REMOTEMASTERSTAT=$(sudo -u $PGUSER psql -h$REMOTE postgres -c "SELECT pg_current_xlog_location()"
                          $PGUSER | head -n 3 | tail -n 1) 
